...
@patch("app.briefing.calendar_client.InstalledAppFlow")
@patch("app.briefing.calendar_client.os.path.isfile", return_value=False)
def test_get_credentials_no_token_file_runs_oauth(mock_isfile, mock_flow):
    ...
    with patch("builtins.open", mock_open()):
        from app.briefing.calendar_client import get_credentials
...
@patch("app.briefing.calendar_client.Credentials")
@patch("app.briefing.calendar_client.os.path.isfile", return_value=True)
...
@patch("app.briefing.calendar_client.Request")
@patch("app.briefing.calendar_client.Credentials")
@patch("app.briefing.calendar_client.os.path.isfile", return_value=True)
...
@patch("app.briefing.calendar_client.build")
@patch("app.briefing.calendar_client.get_credentials")
def test_get_todays_events_returns_items(mock_get_creds, mock_build):
    ...
    from app.briefing.calendar_client import get_todays_events
...
def test_get_tasks_returns_tasks_from_all_tasklists(mock_get_creds, mock_build):
    ...
    from app.briefing.calendar_client import get_tasks
    tasks = get_tasks()
